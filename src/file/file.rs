...
impl File {
    pub fn try_load(uri: String) -> Result<Self, FileError>{
        ...
        if path.is_dir() {
            let try_files: Vec<Result<Self, FileError>> = TRYFILES.iter().map(|file| Self::try_load(Path::new(&uri).join(file).to_str().unwrap().to_string())).collect();
        ...
        let extension: Option<String> = path.extension().and_then(OsStr::to_str).map(|ext| ext.to_string());
        ...
        if content.is_ok() {
            ...
            Ok(File { path: path.to_str().unwrap().to_string(), extension, content: content.unwrap(), mime_type, size })
        } else {
